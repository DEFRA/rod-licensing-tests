...
class GetCRMNotify {
  ...
  async fetchCrmNotifyStatus (permissionNumber) {
    return new Promise(function (resolve, reject) {
      ...
      request({
        method: 'GET',
        url: process.env['DYNAMICS_ENDPOINT'] + `/api/data/v9.0/defra_notifications?$expand=defra_PermissionId&$filter=defra_PermissionId/defra_name eq '${permissionNumber}' and defra_status eq 910400001`,
        headers:
          {
