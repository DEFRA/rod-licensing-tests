...
describe('createPermissions tests', () => {
  ;[
    ...
    describe(`tests when today is ${today.toLocaleDateString()}`, () => {
      ...
      it('sets licensee date of birth to be 25 years ago yesterday', async () => {
        const permission = await createPermission()
  ...
  it('generates a permit id using uuid', async () => {
    const sampleUUID = 'abcdefgh-ijkl-mnop-qrst-uvwxyz012345'
    const v4Stub = sinon.stub(uuid, 'v4')
    try {
      v4Stub.onFirstCall().returns(sampleUUID)
      const { permitId } = await createPermission()
      expect(permitId).to.equal(sampleUUID)
    } finally {
      v4Stub.restore()
    }
  })
  ...
  ;[
    ...
    it(`licensee has ${fieldName} matching expected format`, async () => {
      const permission = await createPermission()
