...
describe("generate record tests", () => {
  ...
  [
    { random: 0.999, expectedMopex: 5 },
    { random: 0.001, expectedMopex: 1 },
    { random: 0.438, expectedMopex: 3 },
  ].forEach((test) => {
    it(`MOPEX field is ${test.expectedMopex} for random value of ${test.random}`, () => {
      try {
        sinon.stub(Math, "random").callsFake(() => test.random);
        const {
          REC: { MOPEX },
  ...
  [
    {
      random: 0.999,
      expectedNotifyPost: "Y",
    },
    {
      random: 0.001,
      expectedNotifyEmail: "Y",
      expectedNotifyEmailAddress: "Patrick.Bamford@mailinator.com",
    },
    {
      random: 0.438,
      expectedNotifyEmail: "Y",
      expectedNotifyEmailAddress: "Kalvin.Phillips@mailinator.com",
    },
  ].forEach((test) => {
    it(`Notify fields are as expected for random value of ${test.random}`, () => {
      try {
        sinon.stub(Math, "random").callsFake(() => test.random);
        const {
          REC: { NOTIFY_EMAIL, NOTIFY_EMAIL_ADDRESS, NOTIFY_SMS, NOTIFY_SMS_NUMBER, NOTIFY_POST },
        } = generateRecord(getNames(), getAddresses(), getLicenceTypes());
        expect({
          NOTIFY_EMAIL,
          ...
          NOTIFY_EMAIL: test.expectedNotifyEmail,
          NOTIFY_EMAIL_ADDRESS: test.expectedNotifyEmailAddress,
          NOTIFY_SMS: test.expectedNotifySms,
          NOTIFY_SMS_NUMBER: test.expectedNotifySmsNumber,
          NOTIFY_POST: test.expectedNotifyPost,
        });
      } finally {
        Math.random.restore();
      }
  ...
  [
    {
      random: 0.999,
      expectedCommsOption: "N",
    },
    {
      random: 0.001,
      expectedCommsOption: "Y",
    },
    {
      random: 0.438,
      expectedCommsOption: "N",
    ...
    it(`Comms fields are as expected for random value of ${test.random}`, () => {
      try {
        sinon.stub(Math, "random").callsFake(() => test.random);
        const {
          REC: { COMMS_OPTION },
        ...
        expect({
          ...
          COMMS_OPTION: test.expectedCommsOption,
        });
      } finally {
        Math.random.restore();
      }
...
const getLicenceTypes = () => ["Full Licence", "Senior Concession", "Disabled Concession", "Junior Concession"];
